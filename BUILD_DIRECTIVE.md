...
- Live Firestore: top-level `manhwas` collection, ~1,525 docs, single user (UID `TKTZCY0gXMcfJB1EFI96W1VecuC3`). Plus `userSettings/{uid}` holding `statusConfig`.
...
- ~20 docs have `isFavorite: true`. Owner's definition of favorite: *finished it and would reread it* → migration sets `wouldRevisit = isFavorite`.
- ~113 titles embed alternate names in the title itself, separated by `" / "` (space-slash-space). Slash **without** surrounding spaces (e.g. `1/24 Romance`) is part of the title — never split on it.
- The `notes` field is unused and is being removed from the UI (preserve any non-empty values into the new docs anyway, defensively, but build no UI for it).
- One known duplicate pair exists (`I Am Being Chased By My Husband / Most Wanted Wife`); dedupe logic must handle it generically.
...
5. **AniList enrichment:** for each item, search AniList (type MANGA) by primary title. **Accept a match only if** a normalized title/synonym from the result exactly equals one of the item's normalized titles (primary or alternative). On match: union AniList synonyms into `alternativeTitles` (cap 25), set `coverUrl`, `year`, `externalIds.anilistId`, and set `mediaType` from `countryOfOrigin` (KR→manhwa, CN→manhua, JP→manga; anything else→manhwa). On no confident match: keep item unchanged, `mediaType: 'manhwa'`, `coverUrl: null`, add to report's `unmatched` list. **Rate limit: max 1 request per 2.5 seconds** with exponential backoff on HTTP 429 (honor `Retry-After`). ~1,500 items ≈ 60–70 minutes; print progress every 25 items and write partial state so an interrupted run can resume (cache lookups in `backups/anilist-cache.json`).
...
**Acceptance for Phase 1:** dry-run report on live data reconciles to ~1,525 docs with zero unexplained losses; migrated app shows covers for the large majority of items; add-flow finds "Solo Leveling" (AniList) and "The Departed" (TMDB) with covers; a title containing commas round-trips through add/edit with its title intact; legacy collection unmodified (verify by count).
