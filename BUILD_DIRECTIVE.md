...
4. **Dedupe — exact primary titles only.** Group docs by normalized primary title (lowercase, Unicode-normalize, strip punctuation, collapse whitespace) and merge those groups: union alt titles, OR the favorite flags, keep the earliest `createdAt` and first non-empty note, and keep the "most advanced" status by precedence `Completed > Reading > On Hold > Plan to Read > Dropped`. Log every merge.
   **Do NOT dedupe via legacy alternative titles** — this was tested against the real export and produces false merges: corrupted AI fragments (`"대공님"`, `"Dad"`) and hallucinated aliases transitively link unrelated series (verified example: it merged *I Love Yoo* into a different webtoon). Instead: (a) after enrichment, auto-merge items that resolved to the **same AniList id** (authoritative — same id = same series); (b) any remaining alt-title overlap goes into the report's `possibleDuplicates` list for the owner to resolve by hand in the app. Expected on current data: 17 exact-title merges.
